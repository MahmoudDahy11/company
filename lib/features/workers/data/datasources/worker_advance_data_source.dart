  ...
  Future<void> addAdvance({
    ...
    await _database.transaction(() async {
      final id = await _database
          ...
          .insert(WorkerAdvancesCompanion.insert(
            workerId: workerId, amount: amount,
            date: date, notes: Value(notes),
          ));
      final row = await (_database.select(_database.workerAdvances)
            ..where((t) => t.id.equals(id)))
          .getSingle();
      await queueSync(_database,
        operation: SyncQueueOperation.insert,
        tableName: 'worker_advances', recordId: id, payload: row.toJson(),
      );
    });
  }
  Future<void> addOrUpdateAdvance({
    ...
    await _database.transaction(() async {
      ...
      if (advanceId == null) {
        ...
        await (_database.update(_database.workerAdvances)
              ..where((table) => table.id.equals(advanceId)))
            .write(WorkerAdvancesCompanion(
              workerId: Value(workerId), amount: Value(amount),
              date: Value(date), notes: Value(notes),
            ));
        id = advanceId;
      ...
      final row = await (_database.select(_database.workerAdvances)
          ...
          .getSingle();
      await queueSync(_database,
        operation: op, tableName: 'worker_advances',
        recordId: id, payload: row.toJson(),
      );
    });
  }
  Future<void> deleteAdvance(int advanceId) async {
    final existing = await (_database.select(_database.workerAdvances)
      ..where((table) => table.id.equals(advanceId))).getSingleOrNull();
    if (existing == null) return;
    await _database.transaction(() async {
      await (_database.delete(_database.workerAdvances)
            ..where((table) => table.id.equals(advanceId)))
          .go();
      await queueSync(_database,
        operation: SyncQueueOperation.delete,
        tableName: 'worker_advances', recordId: advanceId,
        payload: <String, dynamic>{'id': advanceId, 'workerId': existing.workerId},
      );
    });
  }
    ...
    await _database.transaction(() async {
      ...
      if (existing == null) {
        id = await _database
            ...
            .insert(WorkerAbsentDaysCompanion.insert(
              workerId: workerId, monthStart: normalizedMonth,
      ...
      final row = await (_database.select(_database.workerAbsentDays)
            ..where((t) => t.id.equals(id)))
          .getSingle();
      await queueSync(_database,
        operation: op, tableName: 'worker_absent_days',
        recordId: id, payload: row.toJson(),
      );
    });
  }
