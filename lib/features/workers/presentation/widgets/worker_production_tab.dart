      ...
      child: SingleChildScrollView(
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: productions.map((item) => DataRow(cells: [
              DataCell(Text(DateFormat.yMd().format(item.date))),
              DataCell(Text(NumberFormat.decimalPattern().format(item.stitchCount))),
              DataCell(Text(currency.format(item.dailyEarnings))),
              DataCell(ConstrainedBox(
                constraints: const BoxConstraints(maxWidth: 200),
                child: Text(item.notes ?? '', overflow: TextOverflow.ellipsis),
              )),
              DataCell(Row(
                    ...
                    onPressed: () async {
                      final result = await showProductionSheet(
                        ...
                        initialValue: ProductionFormResult(
                          productionId: item.id, date: item.date,
                          stitchCount: item.stitchCount, notes: item.notes,
                        ),
                      );
                      if (result == null || !context.mounted) return;
                      await context.read<WorkerDetailsCubit>().saveProduction(
                        productionId: item.id, date: result.date,
                        stitchCount: result.stitchCount, notes: result.notes,
                      );
                    },
                  ...
                  IconButton(
                    onPressed: () async {
                      ...
                      if (confirm == true && context.mounted) {
                        context.read<WorkerDetailsCubit>().deleteProduction(item.id);
                      }
                    },
